...
import { Button, ChevronDown, Edit02, SearchMd } from '@scottylabs/corgi'
    ...
    <div className="flex-1 bg-bg-brandNeutral-secondary-enabled px-10 py-8 overflow-y-auto">
      <div className="max-w-[1120px] mx-auto">
        ...
        <div className="mb-4">
          ...
          {/* Selected courses + Show button row */}
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {schedule?.courses.map((course) => (
                <div
                  key={course.code}
                  className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-stroke-brandNeutral-1 bg-bg-neutral-primary text-xs text-fg-brandNeutral-primary"
                >
                  <span className="font-medium">{course.code}</span>
                  <span className="text-fg-brandNeutral-secondary text-[11px]">
                    {course.section ? `Section ${course.section}` : 'Select section'}
                  </span>
                </div>
              ))}
            </div>
